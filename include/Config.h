...
3. File: src/main.cpp (Diperbarui)


Ini adalah pembaruan besar. Saya telah menambahkan kode Web Server, API handler, dan OTA. Halaman Web UI (HTML/CSS/JS) disematkan langsung dalam kode sebagai string (PROGMEM) agar sederhana.
...
void updateDisplay() { /* ... (Tidak berubah dari Fase 1) ... */ 
   display.clearDisplay();
   ...
   display.printf("P1  : %d siklus (%d ms)", (int)currentLevel_1, (int)currentLevel_1 * MS_PER_CYCLE);
   if (activeSetting == 0) {
   ...
   if (weldMode == 1) {
       display.printf("Jeda: %d siklus (%d ms)", (int)currentLevel_Jeda, (int)currentLevel_Jeda * MS_PER_CYCLE);
       if (activeSetting == 1) {
   ...
   if (weldMode == 1) {
       display.printf("P2  : %d siklus (%d ms)", (int)currentLevel_2, (int)currentLevel_2 * MS_PER_CYCLE);
       if (activeSetting == 2) {
...
// ===================================
// == FUNGSI-FUNGSI LOGIKA (Fase 2) ==
// ===================================

/**
* @brief Inisialisasi WiFi SoftAP
*/
void setupWiFi() {
   Serial.println("Menyiapkan SoftAP...");
...
/**
* @brief Inisialisasi Web Server, API, dan OTA
*/
void setupWebServer() {
   // Handler untuk halaman utama (serve HTML dari PROGMEM)
   server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
       request->send_P(200, "text/html", index_html);
   });

   // API: Mengambil status saat ini
   server.on("/api/state", HTTP_GET, handleGetState);

   // API: Mengatur level baru
   // Menggunakan body parser untuk JSON
   AsyncCallbackJsonWebHandler *handlerSetLevel = new AsyncCallbackJsonWebHandler("/api/level", handleSetLevel);
   server.on(handlerSetLevel);
...
/**
* @brief Handler untuk GET /api/state
* Mengirim status saat ini sebagai JSON
*/
void handleGetState(AsyncWebServerRequest *request) {
   StaticJsonDocument<256> doc;
   doc["mode"] = (int)weldMode;
   doc["p1"] = (int)currentLevel_1;
   doc["jeda"] = (int)currentLevel_Jeda;
   doc["p2"] = (int)currentLevel_2;
   doc["welding"] = welding;
...
/**
* @brief Handler untuk POST /api/level
* Menerima JSON dan memperbarui variabel global
*/
void handleSetLevel(AsyncWebServerRequest *request, JsonVariant &json) {
   if (welding) {
       request->send(503, "text/plain", "Sedang mengelas, tidak bisa mengubah setelan.");
       return;
   }
   ...
   if (obj.containsKey("mode")) {
       weldMode = obj["mode"].as<int>();
       if (weldMode < 0) weldMode = 0;
       if (weldMode > 1) weldMode = 1;
   }
   if (obj.containsKey("p1")) {
       currentLevel_1 = obj["p1"].as<int>();
       if (currentLevel_1 < MIN_LEVEL) currentLevel_1 = MIN_LEVEL;
       if (currentLevel_1 > MAX_LEVEL) currentLevel_1 = MAX_LEVEL;
   }
   if (obj.containsKey("jeda")) {
       currentLevel_Jeda = obj["jeda"].as<int>();
       if (currentLevel_Jeda < MIN_LEVEL) currentLevel_Jeda = MIN_LEVEL;
       if (currentLevel_Jeda > MAX_LEVEL) currentLevel_Jeda = MAX_LEVEL;
   }
   if (obj.containsKey("p2")) {
       currentLevel_2 = obj["p2"].as<int>();
       if (currentLevel_2 < MIN_LEVEL) currentLevel_2 = MIN_LEVEL;
       if (currentLevel_2 > MAX_LEVEL) currentLevel_2 = MAX_LEVEL;
   }

   Serial.println("Setelan diperbarui dari Web UI");
   settingsChanged = true; // Memicu update OLED
   
   // Sinkronkan posisi encoder jika setting aktif berubah
   switch (activeSetting) {
       case 0: encoder.setPosition(currentLevel_1); break;
       case 1: encoder.setPosition(currentLevel_Jeda); break;
       case 2: encoder.setPosition(currentLevel_2); break;
   ...
   handleGetState(request); // Kirim balik status baru sebagai konfirmasi
}

/**
* @brief Handler untuk POST /api/spot
* Memicu fungsi doWeld()
*/
void handleSpot(AsyncWebServerRequest *request) {
   if (welding) {
       request->send(503, "text/plain", "Operasi sedang berlangsung.");
       return;
   }
   
   Serial.println("Pemicu las dari Web UI");
   doWeld();
...
Fase ini berhasil menambahkan lapisan jaringan yang canggih di atas fondasi fisik kita.
* Saat ESP32 boot, ia akan membuat Jaringan WiFi bernama MOT-CycleControl (password: motspotweld).
* Anda bisa terhubung ke WiFi itu dari HP atau laptop Anda.
* Buka browser dan ketik alamat IP (biasanya 192.168.4.1) untuk membuka Web UI.
* Web UI sepenuhnya sinkron dengan OLED. Mengubah slider di HP akan mengubah nilai di OLED (dan sebaliknya, memutar encoder akan mengubah nilai di HP pada poll berikutnya).
* Anda bisa memicu las dari tombol "SPOT" di HP Anda.
* Anda bisa memperbarui firmware dengan mengunjungi alamat 192.168.4.1/update.
* Fitur "Saran" (preset) ada di dalam JavaScript Web UI untuk membantu Anda memulai.
Siap untuk Fase 3 (CI/CD - GitHub Actions)?